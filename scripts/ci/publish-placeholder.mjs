 ...
 *
 * - No NPM_TOKEN / NODE_AUTH_TOKEN (OIDC only for CI; local publish may use token)
 ...
 * - Cache: scripts/.placeholder-npm-cache.json (gitignored)
 ...
 *   repo = oovm/prometheus-downloader
...
const NATIVE_STUBS = [
    ...
    { name: '@doki-land/prometheus-win32-arm64', os: ['win32'], cpu: ['arm64'] },
    { name: '@doki-land/prometheus-darwin-x64', os: ['darwin'], cpu: ['x64'] },
    { name: '@doki-land/prometheus-darwin-arm64', os: ['darwin'], cpu: ['arm64'] },
    { name: '@doki-land/prometheus-linux-x64', os: ['linux'], cpu: ['x64'] },
    { name: '@doki-land/prometheus-linux-arm64', os: ['linux'], cpu: ['arm64'] },
];

/** Placeholder registry version only. Workspace packages stay on 0.0.x until ready. */
const VERSION = '0.0.0';
...
const STUBS = [
    ...
    ...NATIVE_STUBS.map((s) => ({
        ...s,
        description: `Optional native binary for @doki-land/prometheus (${s.name.replace(/^@doki-land\/prometheus-/, '')}). Placeholder only.`,
    })),
];
...
function runNpm(args, opts = {}) {
    ...
    let userConfig;
    if (token) {
        userConfig = path.join(os.tmpdir(), `prometheus-placeholder-npmrc-${process.pid}`);
...
function writeStub(spec, dir, authToken) {
    ...
    const pkg = {
        name: spec.name,
        version: VERSION,
        description: spec.description ?? 'Prometheus placeholder — not for production use.',
        license: 'CC0-1.0',
        private: false,
        files: ['README.md'],
    };
...
function cmdPublish() {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'prometheus-npm-stubs-'));
