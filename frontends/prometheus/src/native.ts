...
export function loadNative(): NativeAddon {
  ...
  const envPath =
    (typeof process.env.PROMETHEUS_NATIVE_NODE === 'string' && process.env.PROMETHEUS_NATIVE_NODE.trim()) || '';
  if (envPath) {
  ...
  throw new Error(
    `prometheus native addon not found for ${name}. Run: pnpm napi:build (writes frontends/prometheus-${short}/)`,
  );
}
