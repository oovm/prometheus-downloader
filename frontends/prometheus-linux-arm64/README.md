...
Optional **native N-API addon** for `@doki-land/prometheus` on **Linux ARM64 (gnu)**.

This package exists so the main JavaScript package stays portable: `@doki-land/prometheus` never ships every OS binary inside one tarball. Instead, npm/pnpm selects this package through `optionalDependencies` when you install on linux/arm64.

## What is inside

| Artifact | Role |
|----------|------|
| `prometheus.linux-arm64-gnu.node` | The **only** native binary in this package |
...
npm will attempt to install `@doki-land/prometheus-linux-arm64` automatically on matching machines. Direct install is fine for debugging:

```bash
pnpm add @doki-land/prometheus-linux-arm64
```
...
Triple / filename: `linux-arm64-gnu` → `prometheus.linux-arm64-gnu.node`.
...
2. `require.resolve('@doki-land/prometheus-linux-arm64/package.json')` sibling binary
3. Workspace / monorepo `frontends/prometheus-linux-arm64/` after `pnpm napi:build`

If nothing loads, run a local build from the monorepo on linux/arm64 (or the CI matrix job for this target):
